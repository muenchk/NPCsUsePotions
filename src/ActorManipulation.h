...
public:
	...
	{
		if (std::get<0>(ActorUseFood(acinfo, static_cast<uint64_t> (Settings::AlchemyEffect::kMagickaRate), raw)) >= 0)
			return true;
		else if (std::get<0>(ActorUseFood(acinfo, static_cast<uint64_t> (Settings::AlchemyEffect::kStaminaRate), raw)) >= 0)
			return true;
		else if (std::get<0>(ActorUseFood(acinfo, static_cast<uint64_t> (Settings::AlchemyEffect::kHealRate), raw)) >= 0)
			return true;
		return false;
	}
